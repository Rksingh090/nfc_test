...
#define NFC4_RFAL_CONV_BITS_TO_BYTES(N) (uint16_t)(((uint16_t)(N) + (NFC4_BITS_IN_BYTE - 1U)) / (NFC4_BITS_IN_BYTE))
 ...
 */
typedef esp_err_t (*nfc4_master_io_t)(struct nfc4_s *, uint8_t, uint8_t *, uint16_t); 
...
typedef struct
{
    ...
    esp_err_t status;                        /**< Current status/error of the transceive. */
} nfc4_rfal_tx_rx_t;
...
{
    gpio_num_t irq; /**< IRQ pin. */

    spi_device_handle_t spi; /**< SPI driver object. */

    gpio_num_t chip_select; /**< Chip select pin descriptor (used for SPI driver). */
...
typedef struct
{
    gpio_num_t scl;  /**< Clock pin descriptor for I2C driver. */
    gpio_num_t sda;  /**< Bidirectional data pin descriptor for I2C driver. */
    gpio_num_t miso; /**< Master input - slave output pin descriptor for SPI driver. */
    gpio_num_t mosi; /**< Master output - slave input pin descriptor for SPI driver. */
    gpio_num_t sck;  /**< Clock pin descriptor for SPI driver. */
    gpio_num_t cs;   /**< Chip select pin descriptor for SPI driver. */
    gpio_num_t irq;  /**< IRQ pin. */
