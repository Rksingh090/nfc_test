...
uint8_t digital_in_read(gpio_num_t pin)
{
    return gpio_get_level(pin);
}

esp_err_t spi_master_write(spi_device_handle_t spi, uint8_t tx_buff[], uint8_t length)
{
    spi_transaction_t t;
    memset(&t, 0, sizeof(t));
    ...
    // t.tx_data[0] = tx_buff[0];
    // t.tx_data[0] = tx_buff[0];
    for (uint8_t cnt = 0; cnt < length; cnt++)
    {
        t.tx_data[cnt] = tx_buff[cnt];
    }

    // printf("Write TX0: 0x%x \n", t.tx_data[0]);
    // printf("Write TX1: 0x%x \n", t.tx_data[1]);
...
esp_err_t spi_master_write_then_read(spi_device_handle_t spi, uint8_t tx_buff[], uint8_t length, uint8_t *data_out, uint8_t len)
{
    spi_transaction_t t;
    memset(&t, 0, sizeof(t));
    ...
    // printf("Read TX0: 0x%x \n", t.tx_data[0]);
    // printf("Read TX1: 0x%x \n", t.tx_data[1]);

    // spi_device_transmit(spi, &t);

    // memcpy(data_out, t.rx_data, len);
...
#define MODE_ENTRY_5_REG(MODE, R0, M0, V0, R1, M1, V1, R2, M2, V2, R3, M3, V3, R4, M4, V4) \
    (MODE >> 8), (MODE & 0xFF), 5, (R0 >> 8), (R0 & 0xFF), (M0), (V0), (R1 >> 8), (R1 & 0xFF), (M1), (V1), (R2 >> 8), (R2 & 0xFF), (M2), (V2), (R3 >> 8), (R3 & 0xFF), (M3), (V3), (R4 >> 8), (R4 & 0xFF), (M4), (V4)
...
#define MODE_ENTRY_6_REG(MODE, R0, M0, V0, R1, M1, V1, R2, M2, V2, R3, M3, V3, R4, M4, V4, R5, M5, V5) \
    (MODE >> 8), (MODE & 0xFF), 6, (R0 >> 8), (R0 & 0xFF), (M0), (V0), (R1 >> 8), (R1 & 0xFF), (M1), (V1), (R2 >> 8), (R2 & 0xFF), (M2), (V2), (R3 >> 8), (R3 & 0xFF), (M3), (V3), (R4 >> 8), (R4 & 0xFF), (M4), (V4), (R5 >> 8), (R5 & 0xFF), (M5), (V5)
...
#define MODE_ENTRY_17_REG(MODE, R0, M0, V0, R1, M1, V1, R2, M2, V2, R3, M3, V3, R4, M4, V4, R5, M5, V5,   \
    ...
    (MODE >> 8), (MODE & 0xFF), 17, (R0 >> 8), (R0 & 0xFF), (M0), (V0), (R1 >> 8), (R1 & 0xFF), (M1), (V1), (R2 >> 8), (R2 & 0xFF), (M2), (V2), (R3 >> 8), (R3 & 0xFF), (M3), (V3), (R4 >> 8), (R4 & 0xFF), (M4), (V4), (R5 >> 8), (R5 & 0xFF), (M5), (V5), (R6 >> 8), (R6 & 0xFF), (M6), (V6), (R7 >> 8), (R7 & 0xFF), (M7), (V7), (R8 >> 8), (R8 & 0xFF), (M8), (V8), (R9 >> 8), (R9 & 0xFF), (M9), (V9), (R10 >> 8), (R10 & 0xFF), (M10), (V10), (R11 >> 8), (R11 & 0xFF), (M11), (V11), (R12 >> 8), (R12 & 0xFF), (M12), (V12), (R13 >> 8), (R13 & 0xFF), (M13), (V13), (R14 >> 8), (R14 & 0xFF), (M14), (V14), (R15 >> 8), (R15 & 0xFF), (M15), (V15), (R16 >> 8), (R16 & 0xFF), (M16), (V16)
...
static const uint8_t nfc4_analog_config_settings[] =
    {
        ...
        MODE_ENTRY_17_REG((NFC4_ANALOG_CONFIG_TECH_CHIP | NFC4_ANALOG_CONFIG_CHIP_INIT), NFC4_REG_IO_CFG_1, (NFC4_IO_CFG_1_OUT_CL_MASK | NFC4_IO_CFG_1_LF_CLK_OFF), 0x07
                          /* Disable MCU_CLK */
                          ,
        ...
        MODE_ENTRY_6_REG((NFC4_ANALOG_CONFIG_POLL | NFC4_ANALOG_CONFIG_TECH_NFCA |
                         ...
                         NFC4_REG_RX_CFG_1, 0xFF, 0x08, NFC4_REG_RX_CFG_2, 0xFF, 0x2D, NFC4_REG_RX_CFG_3, 0xFF, 0x00, NFC4_REG_RX_CFG_4, 0xFF, 0x00, NFC4_REG_CORR_CFG_1, 0xFF, 0x51, NFC4_REG_CORR_CFG_2, 0xFF, 0x00)
...
static esp_err_t nfc4_perform_collision_avoidance(nfc4_t *ctx, uint8_t field_on_cmd,
                                                  uint8_t pd_threshold, uint8_t ca_threshold);

static void nfc4_set_num_tx_bits(nfc4_t *ctx, uint16_t n_bits);

static esp_err_t nfc4_set_no_response_time(nfc4_t *ctx, uint32_t nrt_64fcs);

static esp_err_t nfc4_rfal_iso14443a_transceive_anticollision_frame(nfc4_t *ctx, uint8_t *buf, uint8_t *bytes_to_send,
                                                                    uint8_t *bits_to_send, uint16_t *rx_length, uint32_t fwt);


static esp_err_t nfc4_rfal_iso14443a_transceive_short_frame(nfc4_t *ctx, nfc4_rfal_14443a_short_frame_cmd_t tx_cmd,
                                                            uint8_t *rx_buf, uint8_t rx_buf_len, uint16_t *rx_rcvd_len,
                                                            uint32_t fwt);


static esp_err_t nfc4_rfal_nfca_poller_initialize(nfc4_t *ctx);


static esp_err_t nfc4_rfal_nfca_poller_sleep(nfc4_t *ctx);


static esp_err_t nfc4_rfal_nfca_poller_select(nfc4_t *ctx, uint8_t *nfc_id1, uint8_t nfc_id_len,
                                              nfc4_rfal_nfca_sel_res_t *sel_res);


static esp_err_t nfc4_rfal_nfca_poller_check_presence(nfc4_t *ctx, nfc4_rfal_14443a_short_frame_cmd_t cmd,
                                                      nfc4_rfal_nfca_sens_res_t *sens_res);

static esp_err_t nfc4_rfal_nfca_poller_get_single_collision_resolution_status(nfc4_t *ctx);


static esp_err_t nfc4_rfal_nfca_poller_start_single_collision_resolution(nfc4_t *ctx, bool *coll_pending,
                                                                         nfc4_rfal_nfca_sel_res_t *sel_res,
                                                                         uint8_t *nfc_id1,
                                                                         uint8_t *nfc_id1_len);


static esp_err_t nfc4_rfal_nfca_poller_get_full_collision_resolution_status(nfc4_t *ctx);


static esp_err_t nfc4_rfal_nfca_poller_start_full_collision_resolution(nfc4_t *ctx,
 ...
 */
static esp_err_t nfc4_rfal_nfc_poll_coll_resolution(nfc4_t *ctx);
 ...
 *          @li @c NFC4_RFAL_ERR_XXXX  : Error occurred
 */
static esp_err_t nfc4_rfal_nfc_poll_activation(nfc4_t *ctx, uint8_t dev_idx);
 ...
 */
static esp_err_t nfc4_rfal_nfc_discover(nfc4_t *ctx);
 ...
 */
static esp_err_t nfc4_rfal_nfc_get_active_device(nfc4_t *ctx, nfc4_rfal_nfc_device_t **dev);
 ...
 *          @li @c NFC4_RFAL_ERR_WRONG_STATE  : Incorrect state for this operation
 */
static esp_err_t nfc4_rfal_nfc_deactivate(nfc4_t *ctx, bool discovery);
 ...
 */
static esp_err_t nfc4_rfal_field_on(nfc4_t *ctx);
 ...
 */
static esp_err_t nfc4_rfal_field_off(nfc4_t *ctx);
 ...
 */
static esp_err_t nfc4_rfal_start_transceive(nfc4_t *ctx, nfc4_rfal_transceive_context_t *ctx_tx);
 ...
 */
static esp_err_t nfc4_rfal_transceive_run_blocking_tx(nfc4_t *ctx);
 ...
 *          @li @c NFC4_RFAL_ERR_IO           : Internal error
 */
static esp_err_t nfc4_rfal_transceive_blocking_tx(nfc4_t *ctx, uint8_t *tx_buf, uint16_t tx_buf_len, uint8_t *rx_buf,
 ...
 *          @li @c NFC4_RFAL_ERR_IO           : Internal error
 */
static esp_err_t nfc4_rfal_transceive_blocking_rx(nfc4_t *ctx);
 ...
 *          @li @c NFC4_RFAL_ERR_IO           : Internal error
 */
static esp_err_t nfc4_rfal_transceive_blocking_tx_rx(nfc4_t *ctx, uint8_t *tx_buf, uint16_t tx_buf_len, uint8_t *rx_buf,
                                                     uint16_t rx_buf_len, uint16_t *act_len, uint32_t flags, uint32_t fwt);
 ...
 *          @li @c NFC4_RFAL_ERR_IO           : Internal error
 */
static esp_err_t nfc4_rfal_worker(nfc4_t *ctx);
 ...
 *         @li @c -1 - Error.
 * See #esp_err_t definition for detailed explanation.
 */
static esp_err_t nfc4_set_analog_config(nfc4_t *ctx, uint16_t config_id);
 ...
 * @note None.
 */
static esp_err_t nfc4_spi_write(nfc4_t *ctx, uint8_t reg, uint8_t *data_in, uint16_t len);
 ...
 * @note None.
 */
static esp_err_t nfc4_spi_read(nfc4_t *ctx, uint8_t reg, uint8_t *data_out, uint16_t len);
...
esp_err_t nfc4_init(nfc4_t *ctx, nfc4_cfg_t *cfg)
{
...
esp_err_t nfc4_default_cfg(nfc4_t *ctx)
{
    ctx->int_status = 0;
    ctx->nfc_dev.state = NFC4_NFC_STATE_NOTINIT;

    /* Set default state on the ST25R3916 */
    esp_err_t error_flag = nfc4_send_direct_command(ctx, NFC4_CMD_SET_DEFAULT);
    printf("Sending NFC Direct Command 907 RES %d \n", error_flag);
    ...
    printf("Sending NFC Write Register 910 RES %d \n", error_flag);

    if (NFC4_ERROR == nfc4_check_chip_id(ctx, NULL))
    {
        printf("Sending NFC Check CHip Register 915 \n");
        // return NFC4_ERROR;
    }
    ...
    printf("Sending NFC Check CHip Register 958 RES %d \n", error_flag);
    ...
    printf("Sending NFC Check CHip Register 971 RES %d \n", error_flag);
...
esp_err_t nfc4_generic_write(nfc4_t *ctx, uint8_t reg, uint8_t *data_in, uint16_t len)
{
    return ctx->write_f(ctx, reg, data_in, len);
}

esp_err_t nfc4_generic_read(nfc4_t *ctx, uint8_t reg, uint8_t *data_out, uint16_t len)
{
    return ctx->read_f(ctx, reg, data_out, len);
}

esp_err_t nfc4_send_direct_command(nfc4_t *ctx, uint8_t cmd)
{
    return nfc4_generic_write(ctx, cmd | NFC4_MODE_DIRECT_COMMAND, 0, 0);
}

esp_err_t nfc4_send_direct_command_with_result(nfc4_t *ctx, uint8_t cmd, uint8_t result_reg, uint32_t timeout, uint8_t *result)
{
    esp_err_t error_flag = nfc4_check_for_received_interrupts(ctx);
...
esp_err_t nfc4_write_register(nfc4_t *ctx, uint8_t reg, uint8_t data_in)
{
    return nfc4_generic_write(ctx, reg, &data_in, 1);
}

esp_err_t nfc4_read_register(nfc4_t *ctx, uint8_t reg, uint8_t *data_out)
{
    return nfc4_generic_read(ctx, reg, data_out, 1);
}

esp_err_t nfc4_write_multiple_registers(nfc4_t *ctx, uint8_t reg, uint8_t *data_in, uint16_t len)
{
    return nfc4_generic_write(ctx, reg, data_in, len);
}

esp_err_t nfc4_read_multiple_registers(nfc4_t *ctx, uint8_t reg, uint8_t *data_out, uint16_t len)
{
    return nfc4_generic_read(ctx, reg, data_out, len);
}

esp_err_t nfc4_clear_register_bits(nfc4_t *ctx, uint8_t reg, uint8_t clr_mask)
{
    uint8_t rx_data = 0;
    uint8_t tx_data = 0;

    esp_err_t error_flag = nfc4_read_register(ctx, reg, &rx_data);
...
esp_err_t nfc4_set_register_bits(nfc4_t *ctx, uint8_t reg, uint8_t set_mask)
{
    uint8_t rx_data = 0;
    uint8_t tx_data = 0;
...
esp_err_t nfc4_modify_register_bits(nfc4_t *ctx, uint8_t reg, uint8_t clr_mask, uint8_t set_mask)
{
    uint8_t rx_data = 0;
    uint8_t tx_data = 0;
...
esp_err_t nfc4_write_fifo(nfc4_t *ctx, uint8_t *data_in, uint16_t len)
{
...
esp_err_t nfc4_read_fifo(nfc4_t *ctx, uint8_t *data_out, uint16_t len)
{
    if ((len > NFC4_FIFO_DEPTH) && (0 == len))
    {
        return NFC4_ERROR;
...
esp_err_t nfc4_write_test_register(nfc4_t *ctx, uint8_t reg, uint8_t data_in)
{
    uint8_t tx_buf[2] = {reg, data_in};
    return nfc4_generic_write(ctx, NFC4_CMD_TEST_ACCESS, tx_buf, 2);
}

esp_err_t nfc4_read_test_register(nfc4_t *ctx, uint8_t reg, uint8_t *data_out)
{
    *data_out = reg;
    return nfc4_generic_read(ctx, NFC4_CMD_TEST_ACCESS, data_out, 1);
}

esp_err_t nfc4_modify_test_register_bits(nfc4_t *ctx, uint8_t reg, uint8_t clr_mask, uint8_t set_mask)
{
    uint8_t rx_data = 0;
    uint8_t tx_data = 0;

    esp_err_t error_flag = nfc4_read_test_register(ctx, reg, &rx_data);
...
esp_err_t nfc4_enable_interrupt(nfc4_t *ctx, uint32_t mask)
{
    uint8_t rx_buf[4] = {0};
    uint8_t tx_data = 0;
    esp_err_t error_flag = nfc4_read_multiple_registers(ctx, NFC4_REG_IRQ_MASK_MAIN, rx_buf, 4);
...
esp_err_t nfc4_disable_interrupt(nfc4_t *ctx, uint32_t mask)
{
    uint8_t rx_buf[4] = {0};
    uint8_t tx_data = 0;
    esp_err_t error_flag = nfc4_read_multiple_registers(ctx, NFC4_REG_IRQ_MASK_MAIN, rx_buf, 4);
...
esp_err_t nfc4_clear_interrupts(nfc4_t *ctx)
{
    uint8_t rx_buf[4] = {0};
...
esp_err_t nfc4_check_for_received_interrupts(nfc4_t *ctx)
{
    uint8_t rx_buf[4] = {0};
    esp_err_t error_flag = NFC4_OK;
...
esp_err_t nfc4_check_chip_id(nfc4_t *ctx, uint8_t *revision)
{
    uint8_t chip_id = 0;
    esp_err_t error_flag = nfc4_read_register(ctx, NFC4_REG_IC_IDENTITY, &chip_id);
...
esp_err_t nfc4_enable_osc(nfc4_t *ctx)
{
    uint8_t reg_data = 0;
    esp_err_t error_flag = nfc4_read_register(ctx, NFC4_REG_OP_CTRL, &reg_data);
...
esp_err_t nfc4_measure_voltage(nfc4_t *ctx, uint8_t mpsv_src, uint16_t *res_mv)
{
    uint8_t reg_data = 0;
    ...
    esp_err_t error_flag = nfc4_read_register(ctx, NFC4_REG_REGULATOR_CTRL, &reg_data);
    reg_data &= ~0x07;
    reg_data |= mpsv_src;
...
esp_err_t nfc4_get_mifare_tag_uid(nfc4_t *ctx, uint8_t *uid, uint8_t *uid_len)
{
                                                  ...
                                                  uint8_t pd_threshold, uint8_t ca_threshold)
{
    uint8_t tre_mask = 0;
    uint32_t irqs = 0;
    esp_err_t error_flag = NFC4_RFAL_ERR_INTERNAL;
...
static esp_err_t nfc4_rfal_nfca_poller_initialize(nfc4_t *ctx)
{
    ...
    esp_err_t error_flag = nfc4_clear_register_bits(ctx, NFC4_REG_OP_CTRL, NFC4_OP_CTRL_WU);
...
static esp_err_t nfc4_rfal_nfc_poll_coll_resolution(nfc4_t *ctx)
{
    ...
    esp_err_t error_flag = NFC4_RFAL_ERR_NONE;
...
static void nfc4_rfal_prepare_transceive(nfc4_t *ctx)
{
    ...
    nfc4_modify_register_bits(ctx, NFC4_REG_ISO14443A_NFC, (NFC4_ISO14443A_NFC_NO_TX_PAR | NFC4_ISO14443A_NFC_NO_RX_PAR | NFC4_ISO14443A_NFC_NFC_F0),
...
static void nfc4_rfal_cleanup_transceive(nfc4_t *ctx)
{
    ...
    nfc4_clear_register_bits(ctx, NFC4_REG_ISO14443A_NFC, (NFC4_ISO14443A_NFC_NO_TX_PAR | NFC4_ISO14443A_NFC_NO_RX_PAR | NFC4_ISO14443A_NFC_NFC_F0));
    /* Restore AGC enabled */
...
static void nfc4_rfal_transceive_rx(nfc4_t *ctx)
{
    ...
    case NFC4_TXRX_STATE_RX_READ_FIFO:
    {
        ...
        aux = ((ctx->rfal.fifo.bytes_total > NFC4_RFAL_CONV_BITS_TO_BYTES(ctx->rfal.tx_rx.ctx.rx_buf_len)) ? (NFC4_RFAL_CONV_BITS_TO_BYTES(ctx->rfal.tx_rx.ctx.rx_buf_len) - ctx->rfal.fifo.bytes_written) : tmp);
...
static bool nfc4_rfal_is_transceive_in_tx(nfc4_t *ctx)
{
    return ((ctx->rfal.tx_rx.state >= NFC4_TXRX_STATE_TX_IDLE) && (ctx->rfal.tx_rx.state < NFC4_TXRX_STATE_RX_IDLE));
}
...
static esp_err_t nfc4_rfal_transceive_blocking_tx(nfc4_t *ctx, uint8_t *tx_buf, uint16_t tx_buf_len, uint8_t *rx_buf, uint16_t rx_buf_len, uint16_t *act_len, uint32_t flags, uint32_t fwt)
{
    esp_err_t error_flag;
    nfc4_rfal_transceive_context_t ctx_tx;
...
static esp_err_t nfc4_rfal_transceive_blocking_tx_rx(nfc4_t *ctx, uint8_t *tx_buf, uint16_t tx_buf_len, uint8_t *rx_buf, uint16_t rx_buf_len, uint16_t *act_len, uint32_t flags, uint32_t fwt)
{
    esp_err_t error_flag;
...
static esp_err_t nfc4_spi_write(nfc4_t *ctx, uint8_t reg, uint8_t *data_in, uint16_t len)
{
    ...
    esp_err_t error_flag = NFC4_ERROR;
...
static esp_err_t nfc4_spi_read(nfc4_t *ctx, uint8_t reg, uint8_t *data_out, uint16_t len)
{
    ...
    else if ((NFC4_MODE_PT_MEM_READ != reg) && (NFC4_MODE_FIFO_READ != reg))
    {
